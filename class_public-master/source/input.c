...
int input_shooting(struct file_content * pfc,
  ...
  char * const target_namestrings[] = {"100*theta_s",
                                       ...
                                       "Omega_scf",
                                       "Omega_ini_dcdm",
                                       "omega_ini_dcdm",   //Modification_momentum_transfer{
                                       "alpha_model"};       //Modification_momentum_transfer}
...
int input_read_parameters_species(struct file_content * pfc,
  ...
  double tol_momentum_transfer = 1e-12;
  ...
  if (pba->Omega0_fld != 0.) {
    ...
    if (pba->fluid_equation_of_state == CLP) {
      ...
      class_read_double("alpha_model",pba->alpha_model);
         if ((fabs(pba->alpha_model) > tol_momentum_transfer) && (fabs(pba->wa_fld) > tol_momentum_transfer)) {
             class_stop(errmsg,"inconsistent input: alpha_model and wa_fld cannot both be non-zero in the this version of the code");
         }
        //Modification_momentum_transfer}
    }
