    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      if (pba->has_cdm == _TRUE_) {
        ...
        ppw->pv->y[ppw->pv->index_pt_theta_cdm] = -(1./2.-om*tau/10.)*(pba->alpha_model*pba->H0*pba->cs2_fld)*(pow(ktau_two,4))/(pow(k,4)*16.*pba->Omega0_cdm*(4.+3.*pba->cs2_fld-6.*pba->w0_fld));
...
int perturb_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if (pba->has_cdm == _TRUE_) {
      ...
      /** - ----> newtonian gauge: cdm density and velocity */ //pvecback[pba->index_bg_rho_cdm], pvecback[pba->index_bg_rho_crit]
      ...
      if (ppt->gauge == newtonian) {
        ...
        dy[pv->index_pt_theta_cdm] = - a_prime_over_a*y[pv->index_pt_theta_cdm] + metric_euler + (pba->alpha_model*pow(a,4)*pba->H0/(pba->Omega0_cdm))*(y[pv->index_pt_theta_fld]-y[pv->index_pt_theta_cdm]);
    ...
    /** - ---> fluid (fld) */ //Dark energy equations
    ...
    if (pba->has_fld == _TRUE_) {
      ...
      if (pba->use_ppf == _FALSE_){
        ...
        dy[pv->index_pt_delta_fld] =
          ...
          -9.*(1+w_fld)*(cs2-w_fld)*a_prime_over_a*a_prime_over_a*y[pv->index_pt_theta_fld]/k2;
