...
int perturbations_derivs(double tau,
  ...
  //Modification_momentum_transfer{
        ...
        dy[pv->index_pt_theta_cdm] = - a_prime_over_a*y[pv->index_pt_theta_cdm] + metric_euler + (pba->alpha_model*pow(a,4)*pba->H0/(pba->Omega0_cdm))*(y[pv->index_pt_theta_fld]-y[pv->index_pt_theta_cdm]); /* cdm velocity */
    //Modification_momentum_transfer}
      }
